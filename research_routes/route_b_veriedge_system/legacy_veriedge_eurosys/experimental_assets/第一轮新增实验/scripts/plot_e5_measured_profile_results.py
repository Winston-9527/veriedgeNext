...
def _plot_policy_compare(rows: Sequence[Dict[str, Any]], workload_id: str, output: Path) -> None:
    ...
    workload_field = (
        "expected_challenge_workload_ms_per_task"
        if "expected_challenge_workload_ms_per_task" in rows[0]
        else "mean_verifier_workload_ms_per_task"
    )
    workload = [float(row[workload_field]) for row in rows]
    ...
    panels = [
        ...
        (axes[1][2], workload, "Expected challenge workload", "ms / task", None),
    ]
...
def _plot_pareto(rows: Sequence[Dict[str, Any]], workload_id: str, output: Path) -> None:
    ...
    label_offsets = {
        "random": (8, 10),
        "cost_only": (8, -18),
        "reputation_aware": (-76, 10),
        "network_aware": (-72, -20),
        "risk_constrained": (8, 10),
        "adaptive_verifier": (8, -20),
    }
    ...
    for row in rows:
        ...
        offset = label_offsets.get(policy, (6, 6))
        workload_field = (
            "expected_challenge_workload_ms_per_task"
            if "expected_challenge_workload_ms_per_task" in row
            else "mean_verifier_workload_ms_per_task"
        )
        axes[0].scatter(float(row["goodput_success_per_s"]), float(row["false_dispute_risk"]), s=95, color=color, zorder=3)
        axes[0].annotate(
            label,
            (float(row["goodput_success_per_s"]), float(row["false_dispute_risk"])),
            xytext=offset,
            textcoords="offset points",
            fontsize=8,
            bbox={"boxstyle": "round,pad=0.18", "fc": "white", "ec": "none", "alpha": 0.76},
        )
        axes[1].scatter(float(row["mean_task_latency_s"]), float(row[workload_field]), s=95, color=color, zorder=3)

    right_groups: Dict[tuple[float, float], List[str]] = {}
    for row in rows:
        workload_field = (
            "expected_challenge_workload_ms_per_task"
            if "expected_challenge_workload_ms_per_task" in row
        ...
        key = (round(float(row["mean_task_latency_s"]), 6), round(float(row[workload_field]), 6))
        right_groups.setdefault(key, []).append(POLICY_LABELS.get(str(row["policy"]), str(row["policy"])))

    group_offsets = [(10, 10), (10, -24), (-110, 10), (-110, -24), (10, 34), (-110, 34)]
    for idx, ((latency, workload), labels) in enumerate(sorted(right_groups.items(), key=lambda item: (item[0][0], -item[0][1]))):
        label = " + ".join(labels)
        axes[1].annotate(
            label,
            (latency, workload),
            xytext=group_offsets[idx % len(group_offsets)],
            textcoords="offset points",
            fontsize=8,
    ...
    axes[1].set_title("Latency vs expected challenge workload")
    axes[1].set_xlabel("Mean task latency (s)")
    axes[1].set_ylabel("Expected challenge workload (ms / task)")
