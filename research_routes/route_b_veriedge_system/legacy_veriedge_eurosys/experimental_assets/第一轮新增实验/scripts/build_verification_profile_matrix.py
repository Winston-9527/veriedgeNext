...
NEGATIVE_CONTROL_ATTACK = "same_prompt_old_run_replay"
LIMITATION_ATTACK = "scale_perturbation"
LEGACY_ATTACK_ALIASES = {
    "cross_prompt_stale_substitution": ["cross_prompt_stale_substitution", "stale_replay"],
    "wrong_shard_output": ["wrong_shard_output", "wrong_prompt"],
}
...
def _attack_row(attacks: Mapping[str, Mapping[str, str]], attack_name: str) -> Mapping[str, str]:
    names = [attack_name] + LEGACY_ATTACK_ALIASES.get(attack_name, [])
    for name in names:
        if name in attacks:
            return attacks[name]
    available = ", ".join(sorted(attacks))
    raise KeyError(f"missing attack {attack_name!r}; available attacks: {available}")


def _attack_metric(attacks: Mapping[str, Mapping[str, str]], attack_name: str, field: str) -> float:
    return float(_attack_row(attacks, attack_name)[field])


def _optional_attack_metric(
    attacks: Mapping[str, Mapping[str, str]],
    attack_name: str,
    field: str,
) -> Any:
    if attack_name not in attacks:
        return ""
    return round(float(attacks[attack_name][field]), 6)
...
def main() -> None:
    ...
    for row in selected_rows:
        ...
        material_tprs = [_attack_metric(attacks, name, "detection_rate") for name in PRIMARY_ATTACKS]
        material_locs = [_attack_metric(attacks, name, "localization_acc") for name in PRIMARY_ATTACKS]
        ...
        output_rows.append(
            {
                ...
                "gaussian_tpr": round(_attack_metric(attacks, "gaussian", "detection_rate"), 6),
                "cross_prompt_stale_substitution_tpr": round(_attack_metric(attacks, "cross_prompt_stale_substitution", "detection_rate"), 6),
                "wrong_shard_output_tpr": round(_attack_metric(attacks, "wrong_shard_output", "detection_rate"), 6),
                "layer_skip_tpr": round(_attack_metric(attacks, "layer_skip", "detection_rate"), 6),
                "same_prompt_old_run_replay_tpr": _optional_attack_metric(attacks, NEGATIVE_CONTROL_ATTACK, "detection_rate"),
                "scale_perturbation_tpr": _optional_attack_metric(attacks, LIMITATION_ATTACK, "detection_rate"),
                "gaussian_locacc": round(_attack_metric(attacks, "gaussian", "localization_acc"), 6),
                "cross_prompt_stale_substitution_locacc": round(_attack_metric(attacks, "cross_prompt_stale_substitution", "localization_acc"), 6),
                "wrong_shard_output_locacc": round(_attack_metric(attacks, "wrong_shard_output", "localization_acc"), 6),
                "layer_skip_locacc": round(_attack_metric(attacks, "layer_skip", "localization_acc"), 6),
                "same_prompt_old_run_replay_locacc": _optional_attack_metric(attacks, NEGATIVE_CONTROL_ATTACK, "localization_acc"),
                "scale_perturbation_locacc": _optional_attack_metric(attacks, LIMITATION_ATTACK, "localization_acc"),
                ...
                "material_risk_scope": "gaussian+cross_prompt_stale_substitution+wrong_shard_output+layer_skip",
                "negative_control_scope": NEGATIVE_CONTROL_ATTACK,
                "limitation_scope": LIMITATION_ATTACK,
    ...
    _write_csv(
        ...
        [
            ...
            "gaussian_tpr",
            "cross_prompt_stale_substitution_tpr",
            "wrong_shard_output_tpr",
            "layer_skip_tpr",
            "same_prompt_old_run_replay_tpr",
            "scale_perturbation_tpr",
            "gaussian_locacc",
            "cross_prompt_stale_substitution_locacc",
            "wrong_shard_output_locacc",
            "layer_skip_locacc",
            "same_prompt_old_run_replay_locacc",
            "scale_perturbation_locacc",
            "material_tpr_min",
            "material_tpr_mean",
            "material_locacc_min",
