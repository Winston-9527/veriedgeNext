...
TABLE_CSV = REPO_ROOT / "paper1_veriedge" / "E1" / "tables" / "table4_results.csv"
...
def main() -> None:
    ...
    ax.set_title("Table 4 Cross-Device Paired-Capture Results (Truthful Runtime Labels)")
    ax.legend(loc="upper right")
    _soften_axis(ax)

    png = FIG_DIR / "table4_paired_capture_fpr.png"
    pdf = FIG_DIR / "table4_paired_capture_fpr.pdf"
    fig.savefig(png, dpi=240, bbox_inches="tight")
    fig.savefig(pdf, dpi=240, bbox_inches="tight")
    plt.close(fig)
    print(f"Wrote Table 4 figure to {png}")
